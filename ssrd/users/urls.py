...
router.register(
    r"authorizeCodes", views.AuthorizeCodeViewSet, base_name="authorizeCodes"
)
...
router.register(
    r"projects/(?P<projectId>\d+)/logs", views.ProjectLogViewSet, base_name="logs"
)
...
urlpatterns = [
    ...
    url(
        r"^medias/download/(?P<projectId>\d+)/$", view=views.MediaDownLoadView.as_view()
    ),
