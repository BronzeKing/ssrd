...
class TestCase(APITestCase):
    ...
    def asserter(self, status=200):
        method = self.data.pop("method", None) or methodbyfunc(self._testMethodName) or "get"
        ...
        self.assertEqual(self.response.status_code, status, get_reason(self.response))
        return self
    ...
    def assertResource(self, attrset=["id", "name"]):
        ...
        for attr in attrset:
            ...
            left and self.assertEqual(left, right, "%s is not equal by (%s, %s)" % (attr, left, right))
        return self
